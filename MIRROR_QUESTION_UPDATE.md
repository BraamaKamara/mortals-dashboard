# The Question Mirror Enhancement

## Changes needed in PhilosophicalMirror.jsx:

### 1. Add milestones to mirrorData (around line 47, after capsules: 0):
...
### 2. Load milestones data (around line 165, before capsuleKeys):
...
### 3. Replace "The Question" screen (starts around line 905):

Replace the entire motion.div with key="question" with this enhanced version:
...
>
  ...
  {mirrorData.milestones.all.length > 0 ? (
    ...
    >
      ...
      {/* Values-based question as fallback */}
...
1. **Milestone tracking**: Shows achieved vs pending milestones with visual cards
2. **Progress overview**: Two-column layout showing completion percentage
3. **Gentle tone**: "What Still Calls to You" instead of harsh demands
4. **Contextual questions**: Prompts change based on milestone status
5. **Visual hierarchy**: Uses emerald for achieved, amber for pending
6. **Age calculations**: Shows how far away pending milestones are
7. **Fallback state**: Guides users to Storyline Arc if no milestones exist
8. **Reflection focus**: Changed from "commitment" to "reflection"
