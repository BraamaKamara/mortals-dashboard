...
import {
  ...
  MessageCircle, TrendingUp, Package, FileText, Star, ChevronDown, LogOut, User
} from "lucide-react";
...
export default function MortalsDashboard({ onEnterMirror, currentUser, onLogout }) {
  ...
  const loadProfile = () => {
    try {
      ...
      const avatarEmoji = localStorage.getItem("mortals.profile.avatarEmoji") || "🕯️";
  ...
  return (
    <div className={`${rootBg} min-h-screen`}>
      <div className="mx-auto max-w-6xl p-6 md:p-10 space-y-8">
            ...
            {(dobDate || endDate) && (
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3 pt-2">
                ...
                <div className={`rounded-xl px-3 py-2 border text-sm ${
                  ...
                  <span className="font-semibold">{heroStats ? heroStats.weeksTotal.toLocaleString() : "—"}</span>
                </div>
              </div>
            )}
        ...
        {life && (
          <Card className={`relative overflow-hidden rounded-3xl border-2 shadow-xl ${
            ...
            <CardContent className="space-y-6 relative">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <Stat label="Years Remaining" value={yearsLeftLabel} icon="📅" highlight={true} />
        ...
        <div ref={navSections[3].ref}> {/* Reflection */}
          <Reflector />
        </div>
